...
NCDでは，図\ref{ncd}のようにデータ$x$と$y$で共通するパターンが
...
PRDCでは，$N$個の基底辞書集合$B=\{D_1,D_2,\dots,D_N\}$を最初に定める．そして，データ$x$を
...
例えば，図\ref{cv}のようにA,B,Cというデータがあった場合，2つの圧縮辞書$d_1$と$d_2$
...
%例えば，A，Ｂの2つのデータがあり，Aから作られた辞書を用いて，Bを圧縮した場合，
...
%辞書に基づく非類似度（距離）計算では，データをLZW圧縮アルゴリズムで圧縮
%した際に構成される辞書が利用される．

%辞書構築の全体的な流れは
%図\ref{fig:image/lzw.eps}のようになる．まずデータから文字列を作成する．
%データから文字列を作成する例として，画像処理でよく用いられる方法に，水
%平スキャンがある．
...
%NDDは任意のデータ$x,y$から作成された辞書$D(x),D(y)$に対して次のように定義される．
%\begin{equation}
%NDD(x,y)
%\end{equation}
